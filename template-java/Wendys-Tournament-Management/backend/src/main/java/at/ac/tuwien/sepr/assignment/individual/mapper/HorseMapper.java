...
public class HorseMapper {
  ...
  /**
   * Convert a horse entity object to a {@link HorseParticipantDto}.
   *
   * @param horse the horse to convert
   * @return the converted {@link HorseParticipantDto}
   */
  public HorseParticipantDto entityToDetailDto(Horse horse, Participant participant) {
    LOG.trace("entityToDto({})", horse);
    if (horse == null) {
      return null;
    }

    return new HorseParticipantDto(
        horse.getId(),
        horse.getName(),
        horse.getDateOfBirth(),
